...
const PitchChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Variables for accumulating samples over 1 second.
    let accumulator = 0;
    let sampleCount = 0;
    ...
    const draw = () => {
      ...
      // Accumulate pitch values for one second.
      const now = performance.now();
      accumulator += smoothedPitch;
      sampleCount++;
      if (now - lastUpdateTime >= 1000) {
        const avgPitch = accumulator / sampleCount;
        pitchHistoryRef.current.push(avgPitch);
      ...
      const yMaleUpper = mapPitchToY(maleRangeUpper);
      const yMaleLower = mapPitchToY(maleRangeLower);
      ctx.fillStyle = "rgba(90, 170, 255, 0.2)";
